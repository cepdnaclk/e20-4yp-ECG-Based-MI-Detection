"""
Dataset Evaluation: CNN-LSTM Test Set Performance
Evaluates on test set with breakdown by group
"""
...
def main():
    print(f"\n{'='*60}")
    print("CNN-LSTM EVALUATION - DATASET A")
    print(f"{'='*60}")
    ...
    checkpoint = torch.load('../saved_models/best_model.pth', map_location=device, weights_only=False)
    ...
    # Group comparison (only if multiple MI groups exist)
    if len([g for g in group_metrics.keys() if g != 'pure_normal']) > 0:
    ...
    with open('../results/metrics/evaluation_report.txt', 'w') as f:
        f.write("CNN-LSTM Evaluation Report\n")
        f.write("="*60 + "\n\n")
    ...
    # Adaptive final summary based on what groups are present
    print(f"\n🎯 Key Findings:")
    
    # Check what MI groups we actually have
    mi_groups = [g for g in group_metrics.keys() if g != 'pure_normal']
    
    if len(mi_groups) > 0:
        print(f"\n   MI Groups Evaluated:")
        for mi_group in mi_groups:
            metrics = group_metrics[mi_group]
            n_samples = metrics['n_samples']
            n_detected = int(metrics['recall'] * n_samples)
            n_missed = n_samples - n_detected
            
            print(f"\n   {mi_group.upper()} (n={n_samples}):")
            print(f"      Detected: {n_detected}/{n_samples} cases (missed: {n_missed})")
            print(f"      ROC-AUC:    {metrics['roc_auc']:.4f}")
            print(f"      Recall:     {metrics['recall']:.4f}")
            print(f"      Precision:  {metrics['precision']:.4f}")
            print(f"      F1-Score:   {metrics['f1']:.4f}")
    
    # Special comparison if we have both certain and uncertain
    if 'certain_mi' in group_metrics and 'uncertain_mi' in group_metrics:
        ...
        print(f"\n   📊 Certain vs Uncertain MI Comparison:")
        print(f"      Certain MI   - AUC: {certain_auc:.4f}, Recall: {certain_recall:.4f}")
        print(f"      Uncertain MI - AUC: {uncertain_auc:.4f}, Recall: {uncertain_recall:.4f}")
        print(f"\n   📉 Performance Drop:")
    ...
    elif 'certain_mi' in group_metrics and 'uncertain_mi' not in group_metrics:
        print(f"\n   ℹ️  Note: Test set contains only CERTAIN MI cases")
        print(f"      This evaluation shows performance on diagnostically certain cases only")
        
        if 'pure_normal' in group_metrics:
            fp = group_metrics['pure_normal']['false_positives']
            tn = group_metrics['pure_normal']['true_negatives']
            print(f"\n   📌 False Positive Analysis:")
            print(f"      Normal cases misclassified as MI: {fp}/{fp+tn}")
            print(f"      Specificity: {group_metrics['pure_normal']['specificity']:.4f}")
